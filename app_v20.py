...
# SupplierPass v0.20
# v19 grouped navigation + Documents & Email + restored Onboarding / Approval Queue.

APP_VERSION = "v0.20 full grouped workflow"

source_path = Path(__file__).with_name("app_v19.py")
source_code = source_path.read_text(encoding="utf-8")
definitions = source_code.split("run_grouped_app()", 1)[0]
...
def create_supplier_from_request_safe(request_id, approved_by, notes):
    ...
    if pd.notna(req.get("converted_supplier_id")) and str(req.get("converted_supplier_id")) not in ["", "None", "nan"]:
...
def approval_queue_screen():
    ...
    show_df(view[["request_id", "supplier_name", "category", "requested_by", "expected_annual_spend", "urgency", "status", "approved_by", "approved_at", "converted_supplier_id"]] if not view.empty else view, "No active approval requests.")
    if view.empty:
        return
