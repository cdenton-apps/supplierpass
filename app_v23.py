...
APP_VERSION = "v0.23 expanded demo data"
    ...
    for i in range(1, 13):
        ...
        exec_sql(
            ...
            (
                ...
                supplier_name,
                supplier_key,
                f"{item_prefix}-{(i % 3) + 1:03d}",
                f"Expanded demo item {(i % 3) + 1}",
                f"PO-XD-{supplier_id}-{i:03d}",
                po_date.date().isoformat(),
                promised.date().isoformat(),
                received.date().isoformat(),
                qty,
                round(unit_price, 2),
                round(total, 2),
            ),
...
def load_expanded_demo_data():
    suppliers = [
        ("TRN001", "Joda Freight Ltd", "ops@jodafreight.example", "Transport", "Connor", "Approved", "Active", "Medium", 185000, "Fast and cost-effective regional haulier"),
        ("TRN002", "McDowells Distribution", "traffic@mcdowells.example", "Transport", "Connor", "Approved", "Active", "Low", 142000, "Preferred tail-lift and timed delivery partner"),
        ("TRN003", "Northline Express", "accounts@northline.example", "Transport", "Logistics", "Approval Revoked", "Inactive", "High", 72000, "Service failures; inactive pending ERP update"),
        ("PKG001", "Yorkshire Board Supplies", "quality@yorkshireboard.example", "Packaging", "Quality", "Approved", "Active", "Medium", 245000, "Main board and sheet supplier"),
        ("PKG002", "Pennine Print & Labels", "sales@penninelabels.example", "Packaging", "Procurement", "Approved", "Active", "Low", 88000, "Label and print finishing specialist"),
        ("RAW001", "Calder Adhesives", "technical@calderadhesives.example", "Raw Materials", "Production", "Pending", "Active", "Medium", 64000, "Awaiting insurance evidence"),
        ("MNT001", "Fast Fix Maintenance", "helpdesk@fastfix.example", "Contractor", "Maintenance", "Approved", "Active", "High", 38000, "Useful but higher H&S risk"),
        ("IT001", "CloudOps Software", "security@cloudops.example", "IT / Software", "IT", "On Hold", "Active", "High", 52000, "Cyber review still open"),
        ("UTL001", "GreenGrid Energy", "business@greengrid.example", "Utilities", "Finance", "Approved", "Active", "Low", 120000, "Stable energy provider"),
        ("GEN001", "One-Off Office Supplies", "orders@oneoffoffice.example", "Office Supplies", "Admin", "Dormant", "Inactive", "Low", 2500, "Dormant supplier for inactive testing"),
    ]
    supplier_ids = {}
    for code, name, email, category, owner, approval, app_status, risk, spend, note in suppliers:
        sid = upsert_demo_supplier(code, name, email, category, owner, approval, app_status, risk, spend, f"{DEMO_TAG}: {note}")
    ...
    add_demo_document(supplier_ids["Joda Freight Ltd"], "Goods in Transit Insurance", "2027-02-28", "Accepted", "Valid GIT evidence")
    add_demo_document(supplier_ids["Joda Freight Ltd"], "Public Liability Insurance", "2026-07-31", "Accepted", "Accepted but expiring within review period")
    add_demo_document(supplier_ids["McDowells Distribution"], "Public Liability Insurance", "2027-05-30", "Accepted", "Clean record")
    add_demo_document(supplier_ids["Northline Express"], "Operator Licence", "2025-11-30", "Rejected / Needs replacement", "Rejected licence copy")
    add_demo_document(supplier_ids["Yorkshire Board Supplies"], "ISO 9001 Certificate", "2027-01-31", "Accepted", "Accepted quality certificate")
    add_demo_document(supplier_ids["Yorkshire Board Supplies"], "FSC / PEFC Certificate", "2026-04-30", "Under Review", "Awaiting quality approval")
    add_demo_document(supplier_ids["Calder Adhesives"], "Public Liability Insurance", "2025-12-31", "Uploaded", "Uploaded but not reviewed")
    add_demo_document(supplier_ids["Fast Fix Maintenance"], "RAMS", "2026-03-31", "Under Review", "Needs H&S review")
    add_demo_document(supplier_ids["CloudOps Software"], "Cyber Security Questionnaire", "2026-06-30", "Uploaded", "Awaiting IT security approval")
    add_demo_document(supplier_ids["GreenGrid Energy"], "Supplier Questionnaire", "2027-03-31", "Accepted", "Stable supplier")

    preferences = [
        (supplier_ids["McDowells Distribution"], "Transport", "Preferred for timed/tail-lift deliveries"),
        (supplier_ids["Yorkshire Board Supplies"], "Packaging", "Preferred board supplier due to reliability"),
        (supplier_ids["Pennine Print & Labels"], "Packaging", "Preferred for labels and print finishing"),
        (supplier_ids["GreenGrid Energy"], "Utilities", "Preferred utility provider"),
    ]
    ...
    requests = [
        ("Aire Valley Couriers", "newbiz@airecouriers.example", "Logistics", "Transport", "Backup courier for urgent pallet movements. EXPANDED_DEMO", 30000, "High", "Awaiting Approval"),
        ("Dales Recycling Services", "hello@dalesrecycling.example", "Operations", "Waste / Recycling", "New waste provider for site review. EXPANDED_DEMO", 18000, "Normal", "Draft"),
        ("SecureData Pen Testing", "security@securedata.example", "IT", "IT / Software", "Cybersecurity supplier proposal. EXPANDED_DEMO", 12000, "Normal", "Rejected"),
    ]
    for row in requests:
        exec_sql(
            """
    ...
    email_rows = [
        (supplier_ids["Calder Adhesives"], "Missing document request", "technical@calderadhesives.example", f"{DEMO_TAG} Supplier document request - Calder Adhesives", "Please send updated insurance evidence.", "Drafted", "Procurement"),
        (supplier_ids["Fast Fix Maintenance"], "Expired document chase", "helpdesk@fastfix.example", f"{DEMO_TAG} RAMS review required - Fast Fix Maintenance", "RAMS is awaiting review.", "Sent / manually recorded", "Maintenance"),
        (supplier_ids["CloudOps Software"], "Cyber Security Questionnaire", "security@cloudops.example", f"{DEMO_TAG} Cyber review outstanding - CloudOps Software", "Please complete the cyber security questionnaire.", "Drafted", "IT"),
    ]
    ...
    erp_actions = [
        (supplier_ids["Northline Express"], "TRN003", "Northline Express", "SET_SUPPLIER_INACTIVE", f"{DEMO_TAG}: Approval revoked due to service failures", "Active", "Inactive", "Pending Export", "Demo user"),
        (supplier_ids["One-Off Office Supplies"], "GEN001", "One-Off Office Supplies", "SET_SUPPLIER_INACTIVE", f"{DEMO_TAG}: Dormant supplier no longer required", "Active", "Inactive", "Pending Export", "Demo user"),
    ]
...
def demo_data_screen():
    hero("Expanded Demo Data", "Load a richer dataset to test the full SupplierPass workflow.")
    hint("This demo creates suppliers across categories, preferred suppliers, documents, email chases, onboarding requests, PO history, price data and ERP actions. Use Clear first if you want a clean demo refresh.")
    c1, c2 = st.columns(2)
    with c1:
        if st.button("Load expanded demo data", type="primary"):
            load_expanded_demo_data()
            st.success("Expanded demo data loaded.")
            st.rerun()
    with c2:
    ...
    st.markdown(
        """
        - **10 suppliers** across Transport, Packaging, Raw Materials, Contractor, IT, Utilities and Office Supplies
        ...
        - **Price records** for price comparison and 🏆 best-price flags
