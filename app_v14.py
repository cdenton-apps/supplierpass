...
APP_VERSION = "v0.14 duplicate-safe approvals"
...
def create_supplier_from_request_safe(request_id, approved_by, notes):
    ...
    if req["status"] != "Awaiting Approval":
        c.close()
        return None, f"This request is {req['status']}, so it cannot be approved again."

    name_key = normalise_name(req["supplier_name"])
    email_key = normalise_email(req["supplier_email"])
    duplicate = None
    if email_key:
    ...
    if duplicate:
        ...
        cur.execute("""
            ...
            SET status='Converted to Supplier', approval_decision='Approved - linked existing supplier', approval_notes=?, approved_by=?,
    ...
    """, (
        req["supplier_name"], name_key, req["supplier_email"], email_key, req["category"], req["requested_by"],
...
def suppliers_screen():
            ...
            if st.button("Import suppliers", type="primary"):
                ...
                for _, r in data.iterrows():
                    ...
                    """, (
                        ...
                        name, normalise_name(name), email, normalise_email(email),
    ...
    with tab_duplicates:
        hint("This checks likely duplicates by normalised supplier name and exact email. Existing duplicates are highlighted here so you can clean them later.")
        ...
        if st.form_submit_button("Create request", type="primary"):
            if not name:
                ...
                if not existing.empty:
                    st.warning("A matching supplier already exists. The request can still be created, but approval will link to the existing supplier rather than create a duplicate.")
...
def reports_screen():
    ...
    xlsx = DATA_DIR / f"supplierpass_v14_audit_{date.today().isoformat()}.xlsx"
