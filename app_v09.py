...
APP_VERSION = "v0.9 process-driven prototype"
SUPPLIER_STATUSES = ["Approved", "Pending", "Blocked", "Dormant", "On Hold"]
DOC_STATUSES = ["Uploaded", "Under Review", "Accepted", "Rejected / Needs replacement"]
...
def document_status(expiry, warning=60, missing=False, critical=True, review_status="Accepted"):
    if missing:
    ...
    for _, rule in rules.iterrows():
        ...
        if matching.empty:
            ...
            status = document_status(
                ...
                rule["warning_days"],
                False,
                bool(rule["is_critical"]),
                latest["review_status"] or "Uploaded",
            )
            issue = ""
            ...
            rows.append(
                {
                    ...
                    "Status": status,
                    "Issue": issue,
                    "Review Status": latest["review_status"],
...
def supplier_readiness(supplier):
    ...
    score = 100 - missing * 18 - needs_review * 10 - expired * 25 - expiring * 8 - high_issues * 15
    ...
    if supplier["approval_status"] == "Blocked" or expired > 0 or high_issues > 0:
        ...
        buy_status = "Dormant"
    elif missing > 0 or expiring > 0 or needs_review > 0:
    ...
    if needs_review:
        reasons.append(f"{needs_review} document(s) awaiting review")
    if expired:
    ...
    return score, buy_status, reasons, missing, needs_review, expired, expiring
...
def supplier_table():
    ...
    for _, supplier in suppliers.iterrows():
        score, buy_status, reasons, missing, needs_review, expired, expiring = supplier_readiness(supplier)
        rows.append(
            {
                ...
                rows.append(
                    {
                        ...
                        "Action": "Review document" if doc["Issue"] == "Needs review" else "Chase supplier",
                    }
                )
...
def today_screen():
    hero("Today in SupplierPass", "Your daily queue: review documents, chase evidence, approve suppliers and fix gaps.")
    suppliers = supplier_table()
    gaps = evidence_gaps()
    ...
    with c2:
        kpi("Documents to review", len(docs_to_review), "uploaded evidence")
    with c3:
        open_approvals = int((~requests["status"].isin(["Approved", "Rejected"])).sum()) if not requests.empty else 0
    ...
    if not docs_to_review.empty:
        st.info("You have uploaded documents waiting to be processed. Go to Document Processing to accept/reject them.")
    elif not gaps.empty:
        st.info("You have evidence gaps. Go to Risk & Compliance to review and chase them.")
    else:
        st.success("No immediate processing tasks found.")

    show_df(gaps.head(20), "No priority evidence gaps found.")


def document_processing_screen():
    hero("Document Processing", "This is where uploaded files become accepted evidence and affect supplier readiness.")
    hint(
        "Uploading a file only stores it. To make it count, process it here: confirm the document type, expiry date and mark it as Accepted. Supplier readiness then recalculates automatically."
    )

    docs = df_sql(
        """
        SELECT d.*, s.supplier_name, s.category
    ...
    show_df(docs[["document_id", "supplier_name", "category", "document_type", "file_name", "expiry_date", "review_status", "uploaded_at"]])

    options = {
        f"{r['supplier_name']} - {r['document_type']} - {r['file_name']} ({r['document_id']})": int(r["document_id"])
        for _, r in docs.iterrows()
    }
    selected = st.selectbox("Select document to process", list(options.keys()))
    ...
    rules = df_sql("SELECT * FROM document_rules WHERE category=? ORDER BY is_critical DESC, document_type", (doc["category"] or "",))
    ...
    st.subheader(f"Processing: {doc['supplier_name']}")
    st.write(f"File: `{doc['file_name']}`")
    st.write(f"Current status: **{doc['review_status']}**")

    with st.form("process_document_form"):
        selected_type = st.selectbox("Confirmed document type", doc_types, index=doc_types.index(doc["document_type"]) if doc["document_type"] in doc_types else 0)
        custom_type = ""
        if selected_type == "Other":
            custom_type = st.text_input("Other document type")
        expiry = st.date_input("Confirmed expiry date", value=parse_date(doc["expiry_date"]))
        review_status = st.selectbox(
            "Review decision",
            DOC_STATUSES,
            index=DOC_STATUSES.index(doc["review_status"]) if doc["review_status"] in DOC_STATUSES else 0,
        )
        ...
        if submitted:
            final_type = custom_type.strip() if selected_type == "Other" else selected_type
            if not final_type:
                ...
                exec_sql(
                    ...
                    SET document_type=?, expiry_date=?, review_status=?, reviewed_by=?, review_notes=?, reviewed_at=CURRENT_TIMESTAMP
                    WHERE document_id=?
                    """,
                    (final_type, expiry.isoformat() if expiry else None, review_status, reviewed_by, review_notes, document_id),
                )
                add_timeline(doc["supplier_id"], None, f"Document {review_status}", final_type, reviewed_by)
                st.success("Document processed. Supplier readiness has been recalculated.")
                st.rerun()

    supplier = df_sql("SELECT * FROM suppliers WHERE supplier_id=?", (doc["supplier_id"],)).iloc[0]
    st.subheader("Supplier checklist after processing")
...
def suppliers_screen():
                ...
                for _, r in data.iterrows():
                    ...
                    rows.append(
                        (
                            ...
                            str(r[c_status]).strip() if c_status and pd.notna(r[c_status]) else "Pending",
                            "Medium",
                            spend,
                        )
                    )
    ...
    with tab_profile:
        ...
        if suppliers.empty:
            ...
            score, buy_status, reasons, _, _, _, _ = supplier_readiness(supplier)
            c1, c2, c3 = st.columns(3)
            with c1:
            ...
            with st.form("edit_supplier"):
                ...
                next_review = c.date_input("Next review", value=parse_date(supplier["next_review_date"]) or date.today() + timedelta(days=365))
            ...
            st.subheader("Current checklist")
            show_df(supplier_checklist(supplier), "No document rules configured for this supplier category.")
...
def compliance_screen():
    ...
    tab_gaps, tab_upload, tab_issues, tab_reviews, tab_rules = st.tabs(["Evidence Gaps", "Upload Document", "Issue Log", "Review Cycle", "Document Rules"])

    with tab_gaps:
        show_df(evidence_gaps(), "No evidence gaps.")

    with tab_upload:
        hint("Upload files here. Then go to Document Processing to review and accept them. Until accepted, they show as needing review.")
        suppliers = df_sql("SELECT * FROM suppliers ORDER BY supplier_name")
        if suppliers.empty:
                ...
                if st.form_submit_button("Upload document"):
                    ...
                    if upload and final_type:
                        ...
                        exec_sql(
                            ...
                            (supplier_id, document_type, file_name, file_path, expiry_date, notes, review_status)
                            VALUES (?, ?, ?, ?, ?, ?, ?)
                            """,
                            (supplier_id, final_type, upload.name, str(file_path), expiry.isoformat() if expiry else None, notes, "Uploaded"),
                        )
                        add_timeline(supplier_id, None, "Document uploaded", final_type, "")
    ...
    with tab_rules:
        ...
        show_df(rules, "No document rules yet.")
        with st.form("add_rule"):
            ...
            if st.form_submit_button("Add rule"):
                if category and doc_type:
                    try:
                        exec_sql(
                            "INSERT INTO document_rules (category, document_type, is_critical, warning_days) VALUES (?, ?, ?, ?)",
                            (category, doc_type, int(critical), int(warning_days)),
                        )
...
def onboarding_screen():
    hero("Onboarding Wizard", "Supplier details → checks → evidence → approval.")
    with st.form("wizard"):
...
def reports_screen():
    ...
    with tab_export:
        xlsx = DATA_DIR / f"supplierpass_v09_audit_{date.today().isoformat()}.xlsx"
...
def admin_screen():
    hero("Admin", "Demo mode and implementation checklist.")
    if st.button("Load demo data"):
        load_demo_data()
        st.rerun()
...
page = st.sidebar.radio(
    "Navigation",
    ["Today", "Suppliers", "Document Processing", "Risk & Compliance", "Onboarding Wizard", "Reports", "Admin"],
)
