...
APP_VERSION = "v0.12 status-aware approvals"
...
def upload_document_screen():
    ...
    with st.form("upload_doc"):
        ...
        if st.form_submit_button("Upload document", type="primary"):
            ...
            file_path = folder / upload.name; file_path.write_bytes(upload.getbuffer())
...
def document_processing_screen():
    ...
    """)
    if docs.empty:
        st.info("No documents uploaded yet."); return
    ...
    with st.form("process_doc"):
        ...
        if st.form_submit_button("Save review decision", type="primary"):
            ...
            st.success("Document updated. Supplier readiness has been recalculated."); st.rerun()
...
def approval_queue_screen():
    hero("Approval Queue", "Status-aware approval: drafts are submitted; submitted requests are approved and become suppliers.")
    hint("A request in Draft can only be submitted. A request Awaiting Approval can be rejected or approved. Approval automatically creates the supplier record.")
    ...
    if req["status"] == "Draft":
        ...
        st.warning("This request has been rejected. Create a new request if it needs to be restarted.")
    else:
        st.info(f"No further action is available for status: {req['status']}")
...
def reports_screen():
    ...
    xlsx = DATA_DIR / f"supplierpass_v12_audit_{date.today().isoformat()}.xlsx"
