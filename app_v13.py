...
APP_VERSION = "v0.13 document preview queue"
...
ACTIVE_DOC_STATUSES = ["Uploaded", "Under Review"]
...
def init_db():
    ...
    for table, cols in {
        "suppliers": {"criticality": "TEXT DEFAULT 'Standard'"},
        "supplier_documents": {
            "review_status": "TEXT DEFAULT 'Uploaded'",
            "reviewed_by": "TEXT",
            "review_notes": "TEXT",
...
def apply_style():
    ...
    .review-card{border:1px solid #e5e7eb;border-radius:16px;padding:18px;background:#fff;margin-top:12px}
...
def render_file_preview(file_path, file_name):
    ...
    if not path.exists():
        st.warning("The file record exists, but the physical file could not be found in storage. This can happen on Streamlit Cloud after a redeploy because local files are not permanent.")
        return
    ...
    if suffix in [".png", ".jpg", ".jpeg", ".webp"]:
        ...
        try:
            ...
            st.info("Preview unavailable, but you can download the file above.")
    else:
...
def create_supplier_from_request(req, approved_by, notes):
    ...
    """, (
        req["supplier_name"], req["supplier_email"], req["category"], req["requested_by"],
        float(req["expected_annual_spend"] or 0),
        f"Created from onboarding request {req['request_id']}. {req['reason_needed'] or ''}\nApproval notes: {notes or ''}",
    ))
    exec_sql("""
        ...
        WHERE request_id=?
    """, (notes, approved_by, supplier_id, req["request_id"]))
    add_timeline(supplier_id, req["request_id"], "Approved and supplier created", req["supplier_name"], approved_by)
    return supplier_id
...
def suppliers_screen():
    ...
    tab_import, tab_register = st.tabs(["Import", "Register"])
    with tab_import:
        ...
        if file:
            ...
            if st.button("Import suppliers", type="primary"):
                ...
                st.success(f"Imported {len(rows)} suppliers."); st.rerun()
    with tab_register:
...
def document_processing_screen():
    ...
    hint("Only documents with status Uploaded or Under Review appear in the default queue. Accepted, rejected and archived documents are removed from the working list, but can be shown using the checkbox.")
    ...
    """)
    if docs.empty:
        st.success("No documents are waiting for processing.")
        return
    ...
    with c1:
        ...
        st.write(f"**Current type:** {doc['document_type']}")
        st.write(f"**File:** {doc['file_name']}")
        st.write(f"**Current status:** {doc['review_status']}")
        st.write(f"**Uploaded:** {doc['uploaded_at']}")
    with c2:
        render_file_preview(doc["file_path"], doc["file_name"])
    ...
    with st.form("process_doc"):
        ...
        if st.form_submit_button("Save review decision", type="primary"):
            ...
            if not final_type:
                st.error("Document type is required."); return
            exec_sql("""
            ...
            st.success("Document updated. It has been removed from the working queue unless it remains Uploaded or Under Review.")
            st.rerun()
...
def approval_queue_screen():
    hero("Approval Queue", "Drafts are submitted. Submitted requests are approved and become suppliers.")
    ...
    if req["status"] == "Draft":
        ...
        c1, c2 = st.columns(2)
        if c1.button("Approve & Create Supplier", type="primary"):
            supplier_id = create_supplier_from_request(req, decided_by, notes)
            st.success(f"Approved and supplier created. Supplier ID: {supplier_id}"); st.rerun()
        if c2.button("Reject"):
            exec_sql("""
                UPDATE new_supplier_requests
        ...
        st.success(f"This request has already been approved and converted to supplier ID {req['converted_supplier_id']}.")
...
def reports_screen():
    ...
    xlsx = DATA_DIR / f"supplierpass_v13_audit_{date.today().isoformat()}.xlsx"
...
def admin_screen():
    ...
    if st.button("Load demo data"):
        load_demo_data(); st.success("Demo data loaded"); st.rerun()
    st.warning("Prototype only. Production still needs authentication, tenant separation, secure storage, backups, licensing and support tooling. On Streamlit Cloud, local uploaded files are not permanent after redeploys; production needs proper cloud object storage.")
