...
# SupplierPass v0.21
# Fixes v20 SyntaxError by safely loading v19 definitions with rsplit.
# Keeps grouped navigation, documents/email, onboarding approvals, supplier intelligence and ERP queue.

APP_VERSION = "v0.21 full grouped workflow - syntax fix"

source_path = Path(__file__).with_name("app_v19.py")
source_code = source_path.read_text(encoding="utf-8")

# v20 split at the first run_grouped_app() occurrence, which accidentally cut
# the file at the function definition and caused a SyntaxError. rsplit removes
# only the final app-runner call at the bottom of v19.
if "\nrun_grouped_app()" in source_code:
    definitions = source_code.rsplit("\nrun_grouped_app()", 1)[0]
else:
    definitions = source_code
...
REQUEST_STATUSES = ["Draft", "Awaiting Approval", "Rejected", "Converted to Supplier"]
...
def create_supplier_from_request_safe(request_id, approved_by, notes):
    ...
    """, (
        req["supplier_name"],
        normalise_name(req["supplier_name"]),
        req["supplier_email"] or "",
        normalise_email(req["supplier_email"] or ""),
        req["category"] or "",
        req["requested_by"] or "",
...
def onboarding_screen():
    ...
    with tab_requests:
        ...
        show_df(requests, "No supplier requests yet.")
...
def approval_queue_screen():
    ...
    columns = ["request_id", "supplier_name", "category", "requested_by", "expected_annual_spend", "urgency", "status", "approved_by", "approved_at", "converted_supplier_id"]
    show_df(view[columns] if not view.empty else view, "No active approval requests.")
    if view.empty:
        return
...
def run_grouped_app():
    ...
    if area == "🏠 Home":
        ...
        if page == "Supplier Onboarding":
            onboarding_screen()
        elif page == "Approval Queue":
            approval_queue_screen()
        elif page == "Preferred by Category":
